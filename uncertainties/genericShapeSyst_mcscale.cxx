  ...
  for(unsigned int j=0; j<year.size(); j++){
    for(unsigned int k=0; k<channel.size(); k++){
      ...
      for(int l=0; l<v_root_directories.size(); l++){
        ...
        if(i==0) syst_weight = 0.3; // Diboson
        ...
        hist_genericShapeSyst_up->SetName("M_Zprime_mcscale_up");
        hist_genericShapeSyst_down->SetName("M_Zprime_mcscale_down");
        ...
        //   // save
        //   c1->SaveAs(save_dir + year.at(j) + "/" + channel.at(k) + "/mcscale_"+ v_samples.at(i)+".pdf");
