{
  ...
  TString root_directory = "Zprime_PDFVariations_DNN_output0/";
  ...
  for(int i=0; i<v_samples.size(); i++){
    ...
    for(int j=1; j<Nbins+1; j++){ // bin 0 is underflow -> 1...Nbins+1
      ...
      for(int k=1; k<101; k++){
        TH1F *h_variation = (TH1F*) input_file->Get(root_directory + hist_name + "_PDF_" + to_string(k));
    ...
    TFile *output_file = new TFile(output_directory + "uhh2.AnalysisModuleRunner.MC." + sample + ".root", "RECREATE");
    ...
    legend->AddEntry(h_ratio_up,"PDF_up","l");
    legend->AddEntry(h_ratio_down,"PDF_down","l");
