...
void ExtractSignalNorm(){
  ...
  TString file_dir = "/nfs/dust/cms/user/deleokse/RunII_106_v2/Presel_";
...
vector<bool> isSignalInt (sample_names.size(), false);
  ...
  for(unsigned int j=0; j<year.size(); j++){
    for(unsigned int i=0; i<sample_names.size(); i++){
      //cout << "Sample : " << sample_names.at(i) << " year : " << year.at(j) << endl;
      TString filename;
      ...
      if (!isSignalInt.at(i)) file_out.open("SignalPDFNorm/SignalNorm_"+sample_names.at(i) + "_" + year.at(j) + ".txt");
      else{
          file_out.open("SignalPDFNorm/SignalNorm_"+sample_names.at(i) + "_pos_" + year.at(j) + ".txt");
          file_out_copy.open("SignalPDFNorm/SignalNorm_"+sample_names.at(i) + "_neg_" + year.at(j) + ".txt");
      }   
