...
TString filename = "";
filename_base += "/nfs/dust/cms/user/deleokse/RunII_106_v2/";
...
for(unsigned int j=0; j<year.size(); j++){ 
  for(unsigned int k=0; k<channel.size(); k++)
    for(unsigned int i=0; i<samples.size(); i++){
      ...
      isSignal.at(i) = (samples.at(i).Index("ZPrimeToTT") == 0  || samples.at(i).Index("ALP") == 0  || samples.at(i).Index("RSGluon") == 0 || samples.at(i).Index("Hscalar") == 0 || samples.at(i).Index("Hpseudo") == 0  );
   
      cout << "sample " << samples.at(i) << " is signal " << isSignal.at(i) << endl;

      filename = filename_base  + "DNN_" + year.at(j) +"_" + channel.at(k) +"/uhh2.AnalysisModuleRunner.MC."+ samples.at(i) + ".root";
      ...
      if( isSignal.at(i) ){
         string pdf_numb[100];
         ifstream normfile("/nfs/dust/cms/user/deleokse/RunII_106_v2/CMSSW_10_6_28/src/UHH2/ZprimeSemiLeptonic/macros/src/SignalPDFNorm/SignalNorm_"+ samples.at(i) + "_" + year.at(j) +".txt", ios::in);
         if (normfile.is_open()){
           for(int i = 0; i < 100; ++i){
      ...
      TFile* f_out = new TFile("/nfs/dust/cms/user/deleokse/RunII_106_v2/CMSSW_10_6_28/src/UHH2/ZprimeSemiLeptonic/macros/src/PDF_hists/"+ year.at(j) + "/" + channel.at(k) +"/uhh2.AnalysisModuleRunner.MC." + samples.at(i) +".root", "RECREATE");
      ...
      if( l== 0 && year.at(j)=="UL18" && channel.at(k) == "muon" && samples.at(i)=="TTbar"){
        ...
        c->SaveAs("/nfs/dust/cms/user/deleokse/RunII_106_v2/CMSSW_10_6_28/src/UHH2/ZprimeSemiLeptonic/macros/src/PDF_hists/" + year.at(j) + "/" + channel.at(k) + "/PDF_"+ samples.at(i) + "_" + v_root_directories.at(l) +".pdf");
        c->Close();
      }
      }    
      delete f_in;
