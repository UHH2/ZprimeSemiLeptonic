...
void getRMS(){
  ...
  for(unsigned int i=0; i<samples.size(); i++){
    ...
    // For each sample read the normalization values - one for each of the 100 PDF replicas
    string pdf_numb[100];
    double pdf_norm[100];
    ...
    // Loop over each bin of the Mtt histograms
     for(int j=1; j<26; j++){
      ...
      c->SaveAs("/nfs/dust/cms/user/deleokse/RunII_106_v2/CMSSW_10_6_28/src/UHH2/ZprimeSemiLeptonic/macros/src/PDF_envelopes/PDF_"+ samples.at(i) +".pdf");
      c->Close();
