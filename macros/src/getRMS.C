  ...
  for(unsigned int j=0; j<year.size(); j++){
    ...
    for(unsigned int k=0; k<channel.size(); k++){
      ...
      for(unsigned int i=0; i<samples.size(); i++){
        ...
        if( isSignal.at(i) ){
          string pdf_numb[100];
          ifstream normfile(filename_base + "CMSSW_10_6_28/src/UHH2/ZprimeSemiLeptonic/macros/src/SignalPDFNorm/SignalNorm_"+ samples.at(i) + "_" + year.at(j) +".txt", ios::in);
          if (normfile.is_open()){
            for(int i = 0; i < 100; ++i){
        ...
        TFile* f_out = new TFile(filename_base + "CMSSW_10_6_28/src/UHH2/ZprimeSemiLeptonic/macros/src/PDF_hists/"+ year.at(j) + "/" + channel.at(k) +"/uhh2.AnalysisModuleRunner.MC." + samples.at(i) +".root", "RECREATE");
        for(int l=0; l<v_root_directories.size(); l++){
          ...
          //
          //
          // c->SaveAs(filename_base + "CMSSW_10_6_28/src/UHH2/ZprimeSemiLeptonic/macros/src/PDF_hists/" + year.at(j) + "/" + channel.at(k) + "/PDF_"+ samples.at(i) + "_" + v_root_directories.at(l) +".pdf");
          // c->Close();
          // }
        }
        delete f_in;
