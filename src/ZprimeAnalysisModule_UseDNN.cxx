...
class ZprimeAnalysisModule_UseDNN : public ModuleBASE {

public:
  explicit ZprimeAnalysisModule_UseDNN(uhh2::Context&);
...
//  unique_ptr<Selection> TwoDCut_selection;
  ...
  Event::Handle<float> h_out0; 
  Event::Handle<float> h_out1; 
  Event::Handle<float> h_out2; 
...
void ZprimeAnalysisModule_UseDNN::book_histograms(uhh2::Context& ctx, vector<string> tags){
...
void ZprimeAnalysisModule_UseDNN::fill_histograms(uhh2::Event& event, string tag){
  string mytag = tag + "_Skimming";
...
ZprimeAnalysisModule_UseDNN::ZprimeAnalysisModule_UseDNN(uhh2::Context& ctx){
  //  debug = true;
  debug = false;
 ...
 // }
  ...
  h_out0 = ctx.declare_event_output<float> ("out0");
  h_out1 = ctx.declare_event_output<float> ("out1");
  h_out2 = ctx.declare_event_output<float> ("out2");
...
bool ZprimeAnalysisModule_UseDNN::process(uhh2::Event& event){
  ...
  event.set(h_out0,-100);
  event.set(h_out1,-100);
  event.set(h_out2,-100);
...
UHH2_REGISTER_ANALYSIS_MODULE(ZprimeAnalysisModule_UseDNN)
