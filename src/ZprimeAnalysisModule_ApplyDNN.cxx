...
class ZprimeAnalysisModule_ApplyDNN : public ModuleBASE {

public:
  explicit ZprimeAnalysisModule_ApplyDNN(uhh2::Context&);
...
void ZprimeAnalysisModule_ApplyDNN::book_histograms(uhh2::Context& ctx, vector<string> tags){
...
void ZprimeAnalysisModule_ApplyDNN::fill_histograms(uhh2::Event& event, string tag){
  string mytag = tag + "_Skimming";
...
ZprimeAnalysisModule_ApplyDNN::ZprimeAnalysisModule_ApplyDNN(uhh2::Context& ctx){
  //  debug = true;
  debug = false;
...
bool ZprimeAnalysisModule_ApplyDNN::process(uhh2::Event& event){
...
//    fill_histograms(event, "Trigger");
  ...
  inputs.tensor<float, 2>()(0,3) = event.get(h_DeltaR_j1lep_j1had);
  inputs.tensor<float, 2>()(0,4) = event.get(h_DeltaR_tlep_thad);
  inputs.tensor<float, 2>()(0,5) = event.get(h_JetHadAK4_1j_eta);
  inputs.tensor<float, 2>()(0,6) = event.get(h_JetHadAK4_1j_phi);
  inputs.tensor<float, 2>()(0,7) = event.get(h_JetHadAK4_1j_pt);
  inputs.tensor<float, 2>()(0,8) = event.get(h_JetHadAK4_2j_eta);
  inputs.tensor<float, 2>()(0,9) = event.get(h_JetHadAK4_2j_phi);
  inputs.tensor<float, 2>()(0,10) = event.get(h_JetHadAK4_2j_pt);
  inputs.tensor<float, 2>()(0,11) = event.get(h_JetHadAK8_eta);
  inputs.tensor<float, 2>()(0,12) = event.get(h_JetHadAK8_phi);
  inputs.tensor<float, 2>()(0,13) = event.get(h_JetHadAK8_pt);
  inputs.tensor<float, 2>()(0,14) = event.get(h_JetLep_eta);
  inputs.tensor<float, 2>()(0,15) = event.get(h_JetLep_phi);
  inputs.tensor<float, 2>()(0,16) = event.get(h_JetLep_pt);
  inputs.tensor<float, 2>()(0,17) = event.get(h_Lep_eta);
  inputs.tensor<float, 2>()(0,18) = event.get(h_Lep_phi);
  ...
  //cout << "output2 = " << outputs[0].tensor<float, 2>()(0,2) << '\n';
    ...
    for(int i=0; i<3; i++){
      dnnout_vector.emplace_back(outputs[0].tensor<float, 2>()(0,i));
    }
...
UHH2_REGISTER_ANALYSIS_MODULE(ZprimeAnalysisModule_ApplyDNN)
