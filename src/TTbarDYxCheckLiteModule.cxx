...
class TTbarDYxCheckLiteModule : public ModuleBASE {

 public:
  explicit TTbarDYxCheckLiteModule(uhh2::Context&);
 ...
 protected:
  ...
  std::unique_ptr<Hists> lumihists_tag; std::unique_ptr<Hists> lumihists_probe; std::unique_ptr<Hists> lumihists_final;
};

TTbarDYxCheckLiteModule::TTbarDYxCheckLiteModule(uhh2::Context& ctx){
  ...
  //   std::string log("TTbarDYxCheckLiteModule::TTbarDYxCheckLiteModule -- ");
  //   log += "TTbarDYxCheckLiteModule::TTbarDYxCheckLiteModule -- invalid argument for 'channel' key in xml file (must be 'muon' or 'elec'): \""+channel+"\"";
  ...
  //   //triangul_cut = false;
  // }
  // else throw std::runtime_error("TTbarDYxCheckLiteModule::TTbarDYxCheckLiteModule -- undefined \"keyword\" argument in .xml configuration file: "+keyword);
  //
  ...
  //// OBJ CLEANING
  const     MuonId muoSR(AndId<Muon>    (PtEtaCut  (muon1_pt, 2.1), MuonIDTight()));
  ...
  jet_cleaner1.reset(new JetCleaner(ctx, 15., 5.0));
  jet_cleaner2.reset(new JetCleaner(ctx, 15., 5.0));

  //// EVENT SELECTION
  jet2_sel.reset(new NJetSelection(2, -1, JetId(PtEtaCut(jet2_pt, 5.0))));
  jet1_sel.reset(new NJetSelection(1, -1, JetId(PtEtaCut(jet1_pt, 5.0))));
  ...
  lumihists_final.reset(new LuminosityHists(ctx, "lumi_final"));
...
bool TTbarDYxCheckLiteModule::process(uhh2::Event& event){
...
// std::cout<<" Went through JEC "<<std::endl;
  ...
  // if (event.muons->size()>0) return false; //skip events with muons
  // pass_dilep = ((event.electrons->size()==2) && (event.electrons->at(0).charge()!=event.electrons->at(1).charge()));  //Z->ee
  // pass_dilep = ((event.electrons->size()==2) && (event.electrons->at(0).charge()!=event.electrons->at(1).charge()) && 
  // 		(event.electrons->at(0).v4()+event.electrons->at(1).v4()).M()>60 && (event.electrons->at(0).v4()+event.electrons->at(1).v4()).M()<130);  //Z->ee
  if (event.electrons->size()>0) return false; //skip events with muons
  pass_dilep = ((event.muons->size()==2) && (event.muons->at(0).charge()!=event.muons->at(1).charge()));  //TT->mumu (opposite sign)
  ...
  //  bool pass_Zll = (event.electrons->at(0).v4()+event.electrons->at(1).v4()).M()>60 && (event.electrons->at(0).v4()+event.electrons->at(1).v4()).M()<130;
  //  bool pass_Zll = (event.electrons->at(0).v4()+event.electrons->at(1).v4()).M()>80 && (event.electrons->at(0).v4()+event.electrons->at(1).v4()).M()<100;
  //  bool pass_Zll = (event.electrons->at(0).v4()+event.electrons->at(1).v4()).M()>80 && (event.electrons->at(0).v4()+event.electrons->at(1).v4()).M()<100;
  bool pass_Zll = true; //place-holder
  if(!pass_Zll) return false;
  HFolder("dilep_mass")->fill(event);
  if(event.isRealData) lumihists_final->fill(event);
  ...
  //   std::cout<<" MET = "<<event.met->pt()<<" "<<event.met->uncorr_pt()<<std::endl;
  //   //    std::cout<<" ele1 pt = "<<event.electrons->at(0).pt()<<" ele2 pt = "<<event.electrons->at(1).pt()<<std::endl;
  //   std::cout<<" mu1 pt = "<<event.muons->at(0).pt()<<" mu2 pt = "<<event.muons->at(1).pt()<<std::endl;
  // }
  // std::cout<<" --- END --- "<<std::endl;
  return true;
}

UHH2_REGISTER_ANALYSIS_MODULE(TTbarDYxCheckLiteModule)
