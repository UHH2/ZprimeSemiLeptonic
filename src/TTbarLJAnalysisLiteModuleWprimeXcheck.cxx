...
class TTbarLJAnalysisLiteModuleWprimeXcheck : public ModuleBASE {

 public:
  explicit TTbarLJAnalysisLiteModuleWprimeXcheck(uhh2::Context&);
 ...
 protected:
  ...
  //W+Jets MVA Related, copied from https://github.com/isando3/ZprimeSemiLeptonic/blob/Summer17/src/TTbarLJAnalysisLiteModuleWprimeXcheck.cxx
...
float TTbarLJAnalysisLiteModuleWprimeXcheck::delta_phi(const float phi1, const float phi2){
...
TTbarLJAnalysisLiteModuleWprimeXcheck::TTbarLJAnalysisLiteModuleWprimeXcheck(uhh2::Context& ctx){
  ...
  if     (channel == "muon") channel_ = muon;
    ...
    std::string log("TTbarLJAnalysisLiteModuleWprimeXcheck::TTbarLJAnalysisLiteModuleWprimeXcheck -- ");
    log += "TTbarLJAnalysisLiteModuleWprimeXcheck::TTbarLJAnalysisLiteModuleWprimeXcheck -- invalid argument for 'channel' key in xml file (must be 'muon' or 'elec'): \""+channel+"\"";
...
//       throw std::runtime_error("TTbarLJAnalysisLiteModuleWprimeXcheck::TTbarLJAnalysisLiteModuleWprimeXcheck -- undefined working-point for \""+keyword+"\" in \"elec\" channel");
//     }
//   }
...
//       throw std::runtime_error("TTbarLJAnalysisLiteModuleWprimeXcheck::TTbarLJAnalysisLiteModuleWprimeXcheck -- undefined working-point for \""+keyword+"\" in \"muon\" channel");
//     }
//     else if(channel_ == elec){
...
//     }
//   }
  else throw std::runtime_error("TTbarLJAnalysisLiteModuleWprimeXcheck::TTbarLJAnalysisLiteModuleWprimeXcheck -- undefined \"keyword\" argument in .xml configuration file: "+keyword);
...
bool TTbarLJAnalysisLiteModuleWprimeXcheck::process(uhh2::Event& event){
  ...
  */
  ...
  // jet
  if(!(event.jets->size() >= 2)) throw std::runtime_error("TTbarLJAnalysisLiteModuleWprimeXcheck::process -- logic error: jet multiplicity < 2 ("+std::to_string(event.jets->size())+")");
 ...
 event.set(tt_NDaughters_jet2,NDaughters_jet2);
  ...
  // ttbar reco hyp
  const ReconstructionHypothesis* rec_ttbar = get_best_hypothesis(ttbar_hyps, "Chi2");
  if(!rec_ttbar) throw std::runtime_error("TTbarLJAnalysisLiteModuleWprimeXcheck::process -- logic error: ttbar reconstruction hypothesis (\"get_best_hypothesis\", discr=Chi2) not found");
...
UHH2_REGISTER_ANALYSIS_MODULE(TTbarLJAnalysisLiteModuleWprimeXcheck)
